...
class ModelProvider():
    def __init__(self):
        ...
        parser.add_argument('--light-db-file', type=str,
                            default="",
...
class QueryModelHandler(BaseHandler):
    ...
    def guess_rooms(self, state, walls):
                ...
                if room['given'] is not None:
                    ...
                    for check_room_cand in room['check_rooms']:
                        ...
                        if check_room['given'] is not None:
                            guesses = self.model_wrapper.get_neighbor_rooms(check_room['given'])
        ...
        for room in rooms.values():
            ...
            room['rooms'] = list(set([r['room_id'] for r in room['guesses'] if r['room_id'] != 305]))[:6]
            del room['guesses']
    ...
    def guess_contents(self, room_guesses):
        for room in room_guesses.values():
            ...
            room['characters'] = list(set(c['character_id'] for c in room['characters']))
