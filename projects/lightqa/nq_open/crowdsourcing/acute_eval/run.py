...
from parlai_internal.projects.light.lightqa.nq_open.crowdsourcing.acute_eval.generate_pairings import (
    generate_pairings,
)
...
class NQLightFastAcuteExecutor(LightFastAcuteExecutor):
    def _load_pairings_file(self):
        # Generate pairings for NQ.
        ...
        if not os.path.exists(self.pairings_filepath) or usr_decision == "y":
            ...
            generate_pairings(
                input_dir=self.fast_acute_args.dialogues_input_dir,
                output_file=self.pairings_filepath,
                add_knowledge=self.fast_acute_args.add_knowledge,
                add_history=self.fast_acute_args.add_history,
                add_answer=False,
                add_qm=True,
            )
...
def main(cfg: DictConfig) -> None:
    runner = NQLightFastAcuteExecutor(cfg)
