...
flags.DEFINE_enum('task',
                  ...
                  'Name of the dataset.')
flags.DEFINE_enum('datatype',
                  'train', ['train', 'valid', 'test'],
                  'Data type split.')
flags.DEFINE_string('output_directory',
                    '/checkpoint/light/data/lightqa/',
...
def safe_huggingface_loading(loading_fn: Callable, retries: int = 5, **kwargs):
    ...
    while True:
        try:
            ...
            logging.info(f'Retry for {retry_count + 1}/{retries} after '
...
class DialogueLoader():
    def __init__(self, task: str = 'light_dialog_wild',
                 datatype: str = 'train',
                 n_data_chunks: int = 1,
                 data_chunk_id: int = 0):
        ...
        self.episodes = self.episodes[
                          ...
                          :(data_chunk_id + 1) * data_chunk_size]
    ...
    def add_summary_questions_to_dialogue(
            ...
            summary_model, qg_model):
        dialogue['summary'] = summary_model(
            dialogue['history'], max_length=100)[0]['summary_text']
    ...
    def generate_history(paragraph_list: List[Dict]):
        ...
        for p in paragraph_list:
            ...
            text = p['text']
            if (p['type'] == '_self_say'
                  or p['type'] == '_self_persona') and self_name:
...
class QuestionGenerator:
    def __init__(
          self, path: str = "mrm8488/t5-base-finetuned-question-generation-ap",
          device: int = 0):

        self.tokenizer = safe_huggingface_loading(
            loading_fn=AutoTokenizer.from_pretrained,
    ...
    def extract_entities(
          sentence, pos=('PROPN', 'NOUN'),
          use_named_entities=True, use_noun_chunks=True):
        doc = nlp(sentence)
        results = []
        if pos:
            for token in doc:
    ...
    def reverse_check(self, answer, context, question, threshold=0.):
...
class OverlapQuestionGenerator:
    ...
    def __call__(self, matching_fn_type: str='large_matching_fn'):
        try:
            matching_fn = {
                'large_matching_fn': self.large_matching_fn
                }[matching_fn_type]
        except ValueError:
        ...
        for dialogue in self.dialogues:
            self.add_overlap(
                dialogue['paragraph_list'], matching_fn)

    @staticmethod
    def large_matching_fn(text: str) -> List[str]:
        return list(set()
            | set(OverlapQuestionGenerator.n_gram_matching(
                text, n=6, remove_sw=False))
            | set(OverlapQuestionGenerator.n_gram_matching(
                text, n=5, remove_sw=False))
            | set(OverlapQuestionGenerator.n_gram_matching(
                text, n=4, remove_sw=False))
            | set(OverlapQuestionGenerator.n_gram_matching(
                text, n=4, remove_sw=True))
            | set(OverlapQuestionGenerator.n_gram_matching(
                text, n=3, remove_sw=True))
            | set(OverlapQuestionGenerator.n_gram_matching(
                text, n=2, remove_sw=True))
            | set(OverlapQuestionGenerator.entities_matching(text))
                )

    @staticmethod
    def add_overlap(paragraph_list: List[Dict[str, Any]],
        ...
        for paragraph in paragraph_list:
            ...
            for match in matching_words:
                if match in matching_lookup:
                    match_str = match if isinstance(match, str)\
                                else ' '.join(match)

                    for matched_paragraph_id in matching_lookup[match]:
                        # Find the sentence it matched.
                        matched_paragraph_text = {
                            p['id']: p for p in paragraph_list}[
                            matched_paragraph_id]['text']
    ...
    def entities_matching(sentence: str, **kwargs):
        return [e.lower() 
                for e in QuestionGenerator.extract_entities(
                    sentence, pos=(), use_named_entities=False,
                    use_noun_chunks=True)]
