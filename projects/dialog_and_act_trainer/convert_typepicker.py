...
def clean(msg):
    ...
    for t in txt.split('\n'):
        if t.startswith('_') and '_object_desc' not in t:
            if t.startswith('_self_act') or t.startswith('_self_emote') or t.startswith('_partner_act') or t.startswith('_partner_emote'):
                last_t = t
                ...
                t = t.replace('_task_speech', '_task_typepicker')
