...
# PREVIOUSLY_DONE_TASKS = ["commonsense_model_chat_bart_grounded_forceinvalid_2"]
# PREVIOUSLY_DONE_TASKS = ["commonsense_model_chat_bart_nogrounded_forceinvalid_2"]
# PREVIOUSLY_DONE_TASKS = ["commonsense_model_chat_bart_nogrounded_forceinvalid_3"]
PREVIOUSLY_DONE_TASKS = ["commonsense_nomodel_chat_allowlist_test_grouped_2"]
...
def get_previously_completed_unit_data():
    ...
    for task_name in PREVIOUSLY_DONE_TASKS:
        ...
        for unit in task_units:
            resp = mephisto_data_browser.get_data_from_unit(unit)['data']['initial_data']['response_text']
...
def run_task(cfg: DictConfig, task_directory: str, world_module=None, only_allow=False):
    ...
    # with open("/checkpoint/alexgurung/tmp/validating_test_data/output_InvalidSelfPlayNarrationTeacher_test_data.pkl", "rb") as f:
    #     task_data = pickle.load(f)

    # with open("/checkpoint/alexgurung/tmp/validating_test_data/output_bart_graph_v2_Sat_Aug__6_InvalidSelfPlayNarrationTeacher_test_data.pkl", "rb") as f:
    # with open("/checkpoint/alexgurung/tmp/validating_test_data/output_bart_nograph_v2_Sat_Aug__6_973_InvalidSelfPlayNarrationTeacher_test_data.pkl", "rb") as f:
    #     task_data = pickle.load(f)
    ...
    # good_obs = set(good_obs.split("\n"))

    # new_task_data = []
    # for t in task_data:
    #     # if t['teacher']['agent_observation'] in good_obs:
    #         # new_task_data.append(t['teacher'])
    #     new_task_data.append({})
    #     new_task_data[-1]['response_text'] = t['response']['text']
    #     new_task_data[-1]['game_text_dropoutless'] = t['teacher']['game_text_dropoutless']
    #     new_task_data[-1]['setting_context_text_dropoutless'] = t['teacher']['setting_context_text_dropoutless']
    #     new_task_data[-1]['action_context_text_dropoutless'] = t['teacher']['action_context_text_dropoutless']
    #     new_task_data[-1]['game_text'] = t['teacher']['game_text']
    #     new_task_data[-1]['setting_context_text'] = t['teacher']['setting_context_text']
    #     new_task_data[-1]['action_context_text'] = t['teacher']['action_context_text_dropoutless']
    #     new_task_data[-1]['current_player'] = t['teacher']['current_player']
    #     new_task_data[-1]['action'] = t['teacher']['action']
    #     new_task_data[-1]['actor'] = t['teacher']['actor']
    ...
    for t in task_data:
        ...
        new_task_data[-1]['source_fname'] = fname
    ...
    # seen_responses = get_previously_completed_unit_data()
    # print(f"Pre previously tasks: {len(task_data)}")
    # task_data = [t for t in task_data if t['response_text'] not in seen_responses]
    # print(f"Post previously tasks: {len(task_data)}")
    # random.shuffle(task_data)
