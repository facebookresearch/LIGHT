...
class ModelChatOnboardWorld(CrowdOnboardWorld):
    ...
    def parley(self):
        print(f"SKIPPING ONBOARDING?: {self.skip_onboarding}")
...
class BaseModelChatWorld(CrowdTaskWorld, ABC):
    ...
    def __add_problem_data_to_utterance(self, p, turn_idx: int):
        ...
        """
        for idk in range(len(self.dialog) - turn_idx):
            self.dialog.append({})
        assert (
    ...
    def parley(self):
        ...
        if self.task_turn_idx == 0:
            ...
            fake_bot_message = {
                ...
                'text': "FORCED BOT TEXT",
                'fake_start': True,
                'agent_idx': 1,
            }
            self.agent.observe(validate(fake_human_message))
            self.bot.observe(validate(fake_human_message))
            self.bot.act()
            self.agent.act()
            self.agent.observe(validate(fake_bot_message))
            self.bot.observe(validate(fake_bot_message))
        ...
        acts = [None, None]
        # for idx, agent in enumerate([self.bot, self.agent]):
        for idx, agent in enumerate([self.agent, self.bot]):
        # for idx, agent in enumerate([self.bot, self.agent]):
            print(f"idx, agent: {idx}, {agent}")
            print(f"self.chat_done: {self.chat_done}")
            if not self.chat_done:
                print("TIMING OUT AGENT?")
                # acts[idx] = self.bot.act()
                acts[idx] = agent.act(timeout=self.max_resp_time)
                # acts[idx] = agent.act(timeout=1)
                # acts[idx] = agent.act()
                # acts[idx] = self.bot.act()
                if (
                    agent == self.bot
                ...
                print(f"SETTING ACTS for idx: {idx}")
                print(acts)
                acts[idx] = Message(
                    Compatibility.maybe_fix_act(acts[idx])
                ).json_safe_payload()
                # self.agent.observe(validate(acts[idx]))
                print(
            ...
            if acts[idx].get('task_data', {}).get('final_rating') is not None:
                print("TASK DATA THING OR FINAL RATING NOT NONE")
                self.chat_done = True
                ...
                self.dialog[turn_idx]['all_ratings'] = acts[idx]['task_data'][
                    'all_ratings'
                ]
                ...
                for other_agent in [self.agent, self.bot]:
                    if other_agent != agent:
                        print(f"OTHER AGENT: {other_agent} OBSERVING")
                        # other_agent.observe(validate(acts[idx]))
                        print(acts[idx])
                        if other_agent == self.bot:
                            "narrate from your perspective: Trainer give ritual dagger to sad townsman"
                            action = acts[idx]['text']
                            current_player = acts[idx]['task_data']['currentPlayer']
                            text_context = acts[idx]['task_data']['textContext']
                            acts[idx]['text'] = text_context + "\n" + f"narrate from your perspective: {current_player} {action}"
                            acts[idx]['episode_done'] = True
                        # self.observe({'text': input_text, 'sender': 'human', 'episode_done': True})
                        # for key, val in [
                        #     ('episode_done', False), 
                        #     ('task_data',{}),
                        #     ('timestamp', datetime.timestamp(datetime.now()))]:
                        #     if key not in acts[idx]:
                        #         acts[idx][key] = val
                        print(validate(acts[idx]))
                        other_agent.observe(validate(acts[idx]))
...
class ModelChatWorld(BaseModelChatWorld):
    ...
    def _run_initial_turn(self) -> None:
        ...
        if self.opt['conversation_start_mode'] == 'blended_skill_talk':
            ...
            print("HI CONVERSATION START MODE")
