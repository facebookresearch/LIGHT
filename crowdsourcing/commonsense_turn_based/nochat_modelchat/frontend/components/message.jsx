...
function RenderChatMessage({ message, mephistoContext, appContext, idx }) {
  ...
  let {action, actor, response_text, current_player} = task_data;
  ...
  const isHuman = message.text.includes("HUMAN");
  const isYou = actor.toLowerCase() === current_player.toLowerCase();
  const isYouText = isYou ? "(you)" : "(not you)";
  ...
  return (
    <MaybeCheckboxChatMessage
      isSelf={isHuman}
      agentName={isHuman ? `${actor} ${isYouText}` : "Game Engine"
        // message.id in currentAgentNames
        //   ? currentAgentNames[message.id]
        //   : message.id
      }
      // message={message.text}
      // message={isHuman ? action : message.text}
      message={isHuman ? action : response_text}
