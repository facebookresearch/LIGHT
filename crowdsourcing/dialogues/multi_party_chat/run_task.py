...
def make_unit_validator(db, block_qual):
    ...
    SPAM_MESSAGE_TIMEOUT = (
        3  # Responding within 3 seconds of own message is likely spam
    )
    SPAM_MESSAGE_FORGIVENESS = (
        0.25  # If less than a quarter are spam, we don't penalize
    )
...
def main(cfg: DictConfig) -> None:
    ...
    shared_state = SharedParlAITaskState(
        ...
        on_unit_submitted=make_unit_validator(
            db, cfg.mephisto.blueprint.block_qualification
        ),
    )
