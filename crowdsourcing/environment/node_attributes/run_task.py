...
def validate_unit(unit):
    ...
    scales = {
        ...
        if k != "custom_attributes" and len(v) == 4
    }
    sorted_scales = [
        sorted([(n, f) for n, f in scale.items()], key=lambda x: x[1])
    ...
    avg_vals = {
        k: sum([scale[k] for scale in scales.values()]) / len(scales)
        for k, _ in sorted_scales[0]
    }
    vals_outside_mean = 0
