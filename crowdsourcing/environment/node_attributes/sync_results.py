...
def get_last_update_time():
    with open(
        os.path.join(REMAINING_TASKS_DIRECTORY, "last_sync.txt"), "r"
    ) as sync_file:
...
def mark_accepted(csv_dfs, unit_data):
    try:
        ...
        target_type = {"objects": "objs", "locations": "rooms", "characters": "chars"}[
            unit_data["data"]["inputs"]["itemCategory"]
        ]
...
def main():
    ...
    unprocessed_units = [
        d for d in units_and_data if d[1]["data"]["times"]["task_end"] > last_update
    ]

    unreviewed_units = [
        d for d in unprocessed_units if d[0].get_status() == AgentState.STATUS_COMPLETED
    ]
    if len(unreviewed_units) != 0:
        print(
            "This script can only be run if there are no tasks that need to be reviewed!"
        )
        return
    ...
    accepted_units = [
        d for d in unprocessed_units if d[0].get_status() == AgentState.STATUS_ACCEPTED
    ]
    rejected_units = [
        d for d in unprocessed_units if d[0].get_status() != AgentState.STATUS_ACCEPTED
    ]
    print(
        f"Processing {len(accepted_units)} accepted units and {len(rejected_units)} passed or rejected units."
    )
    ...
    csv_dfs = {
        name: pd.read_csv(os.path.join(REMAINING_TASKS_DIRECTORY, f"{name}.csv"))
    ...
    for csv_name, df in csv_dfs.items():
        df.to_csv(
            os.path.join(REMAINING_TASKS_DIRECTORY, f"{csv_name}.csv"), index=False
        )

    with open(
        os.path.join(REMAINING_TASKS_DIRECTORY, "last_sync.txt"), "w+"
    ) as sync_file:
