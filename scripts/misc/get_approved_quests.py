from light.data_model.light_database import LIGHTDatabase, DB_STATUS_REVIEW, DB_STATUS_PROD, DB_STATUS_REJECTED
...
with open(os.path.join('/Users/jju/tmp_packages/tmp_packages/light_package', 'character_emojis.json'), 'r') as jsonfile:
            ...
            if len(db_chars) > 1:
                for char in db_chars:
                    if ldb.get_id(char['id'])[0]['status'] == DB_STATUS_REVIEW:
                        if input(f"{char['name']} {char['persona']}, (P)rod, (R)eject:").lower()[0] == 'p':
