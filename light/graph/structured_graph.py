...
class OOGraph(object):
    ...
    def _assert_correct_contains(self):
        ...
        for node in self.get_all_nodes():
            assert node.get_container() is not None, f'Node {node} has None as container'
            container = node.get_container()
            assert node in container.get_contents(), f'Node {node} has not in container {container} contents {container.get_contents()}'
    ...
    def _assert_all_nodes_properly_tracked_dict(self):
        ...
        for check_list in [self.agents, self.objects, self.rooms]:
            for id in check_list:
                assert id in self.all_nodes, f'Node id {id} in type dict but not is all_nodes'
    ...
    def from_json(input_json: str):
        ...
        oo_graph.objects = {x: GraphObject.from_json_dict(node_dicts[x]) for x in object_ids}
        oo_graph.agents = {x: GraphAgent.from_json_dict(node_dicts[x]) for x in agent_ids}
