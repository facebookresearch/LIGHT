...
class RLAgent(LightPolyencoderAgent):
    """
...
class TopKRLAgent(LightPolyencoderAgent):
    ...
    def match_batch(self, batch_reply, valid_inds, output=None):
        ...
        if output.embedding_ctx is not None:
            ...
            # ):
            if output.cands_h is not None:
                for i, ectx, cand_hs in zip(
                    valid_inds, output.embedding_ctx, output.cands_h
                ):
                    batch_reply[i].force_set("embedding_ctx", ectx)
                    batch_reply[i].force_set("cand_h", cand_hs)
                    # batch_reply[i].force_set('cands', cands)
            else:
                for i, ectx in zip(valid_inds, output.embedding_ctx):
                    batch_reply[i].force_set("embedding_ctx", ectx)
                    batch_reply[i].force_set("cand_h", None)
                    # batch_reply[i].force_set('cands', cands)
        return batch_reply
    ...
    def score_candidates(self, batch, cand_vecs, cand_encs=None):
        ...
        if cand_encs is not None and batch.candidate_vecs is None:
            ...
            cand_vecs = None
        # context_h, cands_h = self.model(xs=batch.text_vec, mems=None, cands=cand_vecs)
        cand_vecs = None
        if self.opt["enc_type"] == "text":
            padder = torch.nn.ConstantPad1d((0, 768 - batch.text_vec.size(-1)), 0)
            context_h = padder(batch.text_vec).unsqueeze(1)
        elif self.opt["enc_type"] == "textcand":
            padder = torch.nn.ConstantPad1d((0, 768 - batch.text_vec.size(-1)), 0)
            context_h = padder(batch.text_vec).unsqueeze(1)
            cand_vecs = []
            padder = torch.nn.ConstantPad1d(
                (0, 768 - self.fixed_candidate_vecs.size(-1)), 0
            )
            for obs in batch.observations:
                bat_enc = []
                for l in obs["label_indices"]:
                    bat_enc.append(padder(self.fixed_candidate_vecs[l, :]).unsqueeze(0))
                padder_act = torch.nn.ConstantPad1d(
                    (0, self.opt["act_value"] - len(bat_enc)), 0
                )
                if bat_enc != []:
                    bat_enc = padder_act(
                        torch.cat(bat_enc, dim=0).transpose(1, 0)
                    ).unsqueeze(0)
                else:
                    bat_enc = torch.zeros(768, self.opt["act_value"]).unsqueeze(0)
                cand_vecs.append(bat_enc)
            cand_vecs = torch.cat(cand_vecs, dim=0).transpose(2, 1)
            context_h = torch.cat([context_h, cand_vecs], dim=1)
        elif "modelcand" in self.opt["enc_type"]:
            cand_vecs = []
            # padder = torch.nn.ConstantPad1d((0, 768 - self.fixed_candidate_vecs.size(-1)), 0)
            for obs in batch.observations:
                bat_enc = []
                for l in obs["label_indices"]:
                    bat_enc.append(self.fixed_candidate_vecs[l, :].unsqueeze(0))
                padder_act = torch.nn.ConstantPad1d(
                    (0, self.opt["act_value"] - len(bat_enc)), 0
                )
                bat_enc = padder_act(
                ...
                cand_vecs.append(bat_enc)
            cand_vecs = torch.cat(cand_vecs, dim=0).transpose(2, 1)
            context_h, _, cand_encs = self.model(
                ctxt_tokens=batch.text_vec, cand_tokens=cand_vecs
            )
            # context_h = torch.cat([context_h0, cand_encs], dim=1)
        elif "model" in self.opt["enc_type"]:
            context_h, _, _ = self.model(
                ctxt_tokens=batch.text_vec, cand_tokens=cand_vecs
            )
    ...
    def train_step(self, batch):
        ...
        cands_h, context_h = self.score_candidates(
            batch, cand_vecs, cand_encs=cand_encs
        )

        return Output(
            embedding_ctx=context_h, cands_h=cands_h
        )  # , cands=cand_preds, cand_hs=cand_hs)
...
class VerbAnalysisAgent(LightPolyencoderAgent):
    """TopkRL model is a biencoder that outputs the context embedding
    concatenated with the topk candidate embeddings in a (k+1, emb-size) tensor
    at eval time
    """
...
class TopKRLAgentBi(LightBiencoderAgent):
    """TopkRL model is a biencoder that outputs the context embedding
    concatenated with the topk candidate embeddings in a (k+1, emb-size) tensor
    at eval time
    """
    ...
    def score_candidates(self, batch, cand_vecs, cand_encs=None):
        ...
        if self.opt["enc_type"] == "text":
            ...
            self.opt["enc_type"] == "smallbimodel"
            or self.opt["enc_type"] == "largebimodel"
        ):
            context_h, _ = self.model(xs=batch.text_vec, mems=None, cands=cand_vecs)
            context_h = context_h.unsqueeze(1)
        elif (
            self.opt["enc_type"] == "smallbimodelcands"
            or self.opt["enc_type"] == "largebimodelcands"
        ):
            cand_vecs = []
            # padder = torch.nn.ConstantPad1d((0, 768 - self.fixed_candidate_vecs.size(-1)), 0)
            for obs in batch.observations:
...
class NewClusterEncoderAgent(LightPolyencoderAgent):
    @classmethod
    def history_class(cls):
        return GoalOrientedHistory
