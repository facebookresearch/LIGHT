...
ATTRIBUTE_VALUES = {
    'IS_GETTABLE': {
        0: (
            'not gettable',) if is_simple else (
            'Is not gettable',
            'Can not be acquired',
        ),
        1: (
            'gettable',) if is_simple else (
            'Is gettable',
            'Can be picked up',
        ),
    },
    'IS_DRINK': {
        0: (
            'not drinkable',) if is_simple else (
            'Is not drinkable',
            'One can not drink this',
        ),
        1: (
            'drinkable',) if is_simple else (
            'Is drinkable',
            'One may drink this',
        ),
    },
    'IS_FOOD': {
        0: (
            'not edible',) if is_simple else (
            'Is not edible',
            'Can not be consumed as food',
            'not advisable to eat',
        ),
        1: (
            'edible',) if is_simple else (
            'Is ediable',
    ...
    'IS_WIELDABLE': {
        0: (
            'not wieldable',) if is_simple else (
            'Can not be used as a weapon',
            'Can not be wielded',
            'Is not a weapon',
        ),
        1: (
            'wieldable',) if is_simple else (
            'Can be a weapon',
            'Can be wielded',
            'Is a weapon',
        ),
    ...
    'IS_WEARABLE': {
        0: (
            'not wearable',) if is_simple else (
            'Can not be worn',
            'Is not wearable',
        ),
        1: (
            'wearable',) if is_simple else (
            'Can be worn',
            'Is wearable',
        ),
    },
...
TEACHER_PROMPTS = {
    ...
    'adding_character_persona': [
        ...
        'describe the persona of {name}' if new_prompts else 'what persona does {name} have'
